...
# Ajout du dossier parent au path pour permettre les imports
...
log_file = "resultat_test_us09.txt"
...
class Logger(object):
    def __init__(self, filename=None):
        if filename is None:
            # On enregistre toujours dans le même dossier que ce script (test_US)
            script_dir = os.path.dirname(os.path.abspath(__file__))
            filename = os.path.join(script_dir, "resultat_test_us09.txt")
...
print_log(f"[*] Les logs sont sauvés dans 'test_US/resultat_test_us09.txt'\n")
